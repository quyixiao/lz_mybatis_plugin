...
public class MysqlUtil2ShowCreateTable {
    ...
    public static List<FieldBean> readTableDetail(String tableName) {
        ...
        String url = PMysqlMain.mysql_url;

        // MySQL配置时的用户名
        String user = PMysqlMain.mysql_username;

        // Java连接MySQL配置时的密码
        String password = PMysqlMain.mysql_password;
        ...
        try {
            ...
            String sql = "SELECT COLUMN_NAME columnName, DATA_TYPE dataType, COLUMN_COMMENT columnComment FROM INFORMATION_SCHEMA.COLUMNS "
                    + "WHERE table_name = '"
                    + tableName
                    + "' AND table_schema = '" + PMysqlMain.mysql_dbname + "';";
    ...
    public static Map<String, String> getComments() {
        ...
        try {
            ...
            Statement statement = conn.createStatement();
            // 要执行的SQL语句
            String sql = "show table status from  " + PMysqlMain.mysql_dbname;
            ResultSet rs = statement.executeQuery(sql);
            while (rs.next()) {
                // 选择sname这列数据
