...
public class MysqlUtilTable2XML {
    ...
    public static void printXMLForMap(TablesBean bean) {
        String realName = PMysqlMain.pre + bean.getSpaceName() + "";
        String realNam2 = PMysqlMain.pre + bean.getSpaceName();
        ...
        bf.append("<mapper namespace=\"" + PMysqlMain.dao_package + "." + realName + "Dao\">").append("\n");
        bf.append("\n");
        //     bf.append("    <!-- 通用查询映射结果 -->\n");
...
//       bf.append(TAB).append("</update>").append("\n");
        ...
        FieldBean obj = null;
        for(int i =0;i<bean.getFieldList().size()-1;i++) {
...
//      bf.append("\n");
        ...
        String fileName = PMysqlMain.save_path + "/" + realNam2 + "Dao.xml";

        if (StringUtils.isNotBlank(PMysqlMain.mapper_xml_path)) {
            fileName = PMysqlMain.mapper_xml_path + "/" + realNam2 + "Dao.xml";
        }
        try {
